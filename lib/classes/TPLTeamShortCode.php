...
if (!class_exists('TPLTeamShortCode')):
    ...
    class TPLTeamShortCode
    {
            ...
            if (!isset($atts['layout']) || $new_shortCode || $preview) {
                ...
                if ((!$preview && $new_shortCode) || ($preview && TLPTeam()->verifyNonce())) {
                    ...
                    if ($teamQuery->have_posts()) {
                        ...
                        $class = implode(' ', $class);
                        $html .= $this->layoutStyle($layoutID, $atts, $preview);
                        ...
                        if ($isIsotope) {
                            $html .= sprintf('<div class="button-group sort-by-button-group">
        ...
        private function layoutStyle($layoutID, $scMeta, $preview = false) {
            $style = null;
            $name = !empty($scMeta['name-color']) ? $scMeta['name-color'] : null;
            $designation = !empty($scMeta['designation-color']) ? $scMeta['designation-color'] : null;
            $sd = !empty($scMeta['sd-color']) ? $scMeta['sd-color'] : null;
            if ($name) {
        ...
        public function get_team_old_layout($atts) {
            ...
            if ($teamQuery->have_posts()) {
                ...
                $html .= $this->layoutStyle($layoutID, $atts);
